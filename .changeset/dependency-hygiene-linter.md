...
`@babel/parser|types|traverse|generator`) are now listed explicitly. `.npmrc`
no longer sets `shamefully-hoist`, and CI runs `depcheck` so an undeclared or
unused dependency fails the build.
