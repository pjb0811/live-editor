...
### Replacing one control everywhere: `renderField`

To keep the built-in panel and change only how some fields are drawn, pass
`renderField` to `Live.Dnd`. It is called before the built-in control for
every field, including the keys inside an `object` value and the properties
of each array item, and in a custom panel's `Live.Dnd.Field` as well:
...
<Live.Dnd
  ...
  renderField={({ binding }, builtin) => {
    ...
    if (binding.type === 'color') {
      return <MyColorPicker binding={binding} />;
    }

    // Anything else keeps the built-in control.
    return undefined;
  }}
/>
```

What the function returns decides what renders:

| Return | Result |
| --- | --- |
| `undefined` | The built-in control. |
| a node | That node instead. |
| `null` | Nothing. The field's heading still shows in the built-in panel. |

The second argument, `builtin`, is the control that would have rendered, so
you can wrap it instead of replacing it (`<Hint>{builtin}</Hint>`). Don't
render `<Live.Dnd.Field>` from inside `renderField`: it calls `renderField`
again.

A replacement commits through `binding.onChange` like any other control, so
validate first with `validateBindingValue(binding, value)` if the field
declares constraints.

### Handling edit errors: `onEditError`

When an edit can't be applied, the editor shows a toast and leaves the source
unchanged. Pass `onEditError` to handle it yourself instead. The toast is then
not shown:
...
<Live.Dnd
  ...
  onChange={setValue}
  onEditError={error => {
    notify(error.title, error.description);
    logEditError(error);
  }}
/>
```

`title` and `description` are the toast's own messages. `type` says what
failed:

| `type` | When | Extra fields |
| --- | --- | --- |
| `'update'` | A field edit was rejected, usually because the element's `data-binding` names a `property` or `label` its markup doesn't have. | `id`, `label`, `property`, `failure` (with `failure.reason`) |
| `'parse'` | The selected section, or an `items` value inside it, doesn't parse. | `target` (`'section'` or `'items'`), `error` |
| `'items'` | An array edit would have lost source (a spread, a hole), so it was refused. | — |
...
- Keeping the selection pointed at the right items after every edit, so a
  later bulk action never hits the wrong ones. See
  [Selection after an edit](#selection-after-an-edit).
...
The selection follows the rule in
[Selection after an edit](#selection-after-an-edit).
...
### Selection after an edit

`useItemsEditor` and `useChildrenEditor` share one selection rule. Selection
indices are positions, so whatever moves items has to move or clear the
selection with them:

| What changed the source | Selection afterwards |
| --- | --- |
| `add`, `duplicateSelected` | Unchanged. Both append at the end, so no existing item moves. |
| `moveSelected` | The moved items, at their new positions, so the block can be moved again. |
| `move`, `remove`, `removeSelected` | Cleared. Positions shifted under the selection. |
| An item value edit (`useItemsEditor`) | Unchanged. |
| Anything else: another field, undo, an external `value` | Cleared. The same positions may now hold different items. |
| A refused edit | Unchanged. No new source was produced. |

`useChildrenEditor` applies the result when the new source arrives, since the
host's `onChange` decides whether the edit succeeds. `useItemsEditor` applies
it immediately, because its own array edit has already succeeded by then.
...
text. [Data Binding](./data-binding.mdx) describes how each field is
authored and what the built-in panel does with it.
...
embedded demo above to see it live. To do the same without writing a whole
panel, see [`renderField`](#replacing-one-control-everywhere-renderfield).

Any key you author on a binding that the library doesn't define arrives
under `binding.meta`, typed `unknown` for the same reason.

### Nested fields

A render-map leaf accepts every field a top-level binding does (see
[Nested fields](./data-binding.mdx#nested-fields)). `useItemsEditor` hands
each key of an item back as a `PanelBinding` carrying those fields, so switch
on `widget` and validate against constraints exactly as for a flat binding.
`label` and `property` fall back to the key, and `meta.valueType` is always
set to the property's actual kind (an authored `valueType` is overridden).
