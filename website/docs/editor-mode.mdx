...
- `dynamicTailwind` compiles Tailwind classes from the rendered preview at
  runtime and injects them as a `<style>` tag — needed in `shadow` mode,
  which has no separate document to load a stylesheet into (see
